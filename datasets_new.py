...
class BasicData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        imgdata = imgdata.resize((128, 128), Image.HAMMING)
...
class BuildDataSet(Dataset):
    ...
    def pred_sample(cls, image_sparse, model):
        model.eval()
        with torch.no_grad():
            image_pred = model(torch.from_numpy(image_sparse).unsqueeze_(0).unsqueeze_(0))
    ...
    def __getitem__(self, idx):
        if self.Dataset_name is "train":
            image, image_temp= self.imgset[idx]
        else:
            image, image_name, image_temp = self.imgset[idx]
